namespace RM.Razor {
    ...
    public class MultiTenantRazorViewEngineOptionsSetup : IConfigureOptions<MultiTenantRazorViewEngineOptions> {
        ...
        public void Configure(MultiTenantRazorViewEngineOptions options) {
            ...
            var defaultPageSearchPath = CombinePath(rootDirectory, "{1}/{0}" + MultiTenantRazorViewEngine.ViewExtension);
            options.PageViewLocationFormats.Add(defaultPageSearchPath);

            // /Pages/Shared/{0}.cshtml
            var pagesSharedDirectory = CombinePath(rootDirectory, "Shared/{0}" + MultiTenantRazorViewEngine.ViewExtension);
            options.PageViewLocationFormats.Add(pagesSharedDirectory);

            options.PageViewLocationFormats.Add("/Views/Shared/{0}" + MultiTenantRazorViewEngine.ViewExtension);
            ...
            var areaSearchPath = CombinePath(areaPagesDirectory, "{1}/{0}" + MultiTenantRazorViewEngine.ViewExtension);
            options.AreaPageViewLocationFormats.Add(areaSearchPath);

            var areaPagesSharedSearchPath = CombinePath(areaPagesDirectory, "Shared/{0}" + MultiTenantRazorViewEngine.ViewExtension);
            options.AreaPageViewLocationFormats.Add(areaPagesSharedSearchPath);

            var areaViewsSharedSearchPath = CombinePath(areaDirectory, "Views/Shared/{0}" + MultiTenantRazorViewEngine.ViewExtension);
            ...
            options.AreaPageViewLocationFormats.Add("/Views/Shared/{0}" + MultiTenantRazorViewEngine.ViewExtension);
