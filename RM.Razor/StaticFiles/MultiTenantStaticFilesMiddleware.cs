...
namespace RM.Razor.StaticFiles {
    ...
    /// </summary>
    public class MultiTenantStaticFilesMiddleware {
        ...
        public MultiTenantStaticFilesMiddleware(RequestDelegate next, IWebHostEnvironment hostingEnv, IOptions<StaticFileOptions> options, ILoggerFactory loggerFactory) {
            if (hostingEnv == null) {
        ...
        private Task TryServeStaticFile(HttpContext context, string contentType, PathString subPath) {
            var fileContext = new MultiTenantStaticFileContext(context, _options, _logger, _fileProvider, contentType, subPath);
