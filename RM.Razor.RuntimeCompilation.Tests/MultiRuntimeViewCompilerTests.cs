namespace RM.Razor.RuntimeCompilation.Tests {
    ...
    using Moq;
    using Xunit;
    using static TestRazorCompiledItem;

    public class MultiRuntimeViewCompilerTests {
        [Fact]
...
public class MyTestType
{
    ...
    public string Name
    {
        ...
        set => _name = value ?? throw new System.ArgumentNullException(nameof(value));
            ...
            // Act
            var result = compiler.CompileAndEmit(codeDocument, content, GetEngine(), "test");
...
this should fail";
            ...
            // Act & Assert
            var ex = Assert.Throws<PublicCompilationFailedException>(() => compiler.CompileAndEmit(codeDocument, content, GetEngine(),"test" ));
        ...
        public void CompileAndEmit_DoesNotThrowIfDebugTypeIsEmbedded() {
            ...
            // Act
            var result = compiler.CompileAndEmit(codeDocument, "public class Test{}", GetEngine(), "test");
        ...
        private class TestRazorViewCompiler : RuntimeMultiViewCompiler {
            ...
            public TestRazorViewCompiler(                
                ...
                : base(new Dictionary<string, RazorProjectEngine> { { "default", projectEngine } }, csharpCompiler, precompiledViews, NullLogger.Instance) {
            ...
            protected override CompiledViewDescriptor CompileAndEmit(string relativePath, RazorProjectEngine engine, string assemblyName) {
                return Compile(relativePath);
            }
        }

        private class TestCSharpCompiler : PublicCSharpCompiler {
